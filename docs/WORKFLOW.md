...
Issue 생성 전 `.github/ISSUE_LABELS.md`를 확인하고 기획서를 기준으로 영역·유형·우선순위 Label을 선택한다. AI가 Issue를 생성하는 경우 선택한 Label을 생성 요청에 함께 포함하고, 생성 직후 실제 적용 여부까지 확인한다. 누락된 Label이 있으면 Issue 작업을 시작하기 전에 추가한다.

## 개발 중

- Issue에 연결된 Branch에서만 작업한다.
- 기능 범위를 임의로 넓히지 않는다.
- 오류가 발생하면 담당 영역의 `docs/features/기능명-ERROR.md`에 재현 방법과 원인을 기록한다.
...
- `기능명-REPORT.md`에 변경 내용과 테스트 결과를 기록한다.
