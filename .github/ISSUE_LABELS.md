...
> Issue를 생성하는 사람 또는 AI는 새 기능이면 기획서를, 그 외 작업이면 Issue 내용을 확인하고 아래 기준으로 Label을 선택한다.
...
1. 새 기능이면 관련 `*-PLAN.md`를 읽고, 그 외 작업이면 Issue의 목적과 범위를 확인한다.
2. 담당 영역·대상 앱·작업 유형·개발 우선순위·선행 조건을 확인한다.
...
사용자가 명시적으로 Label 적용을 제외해 달라고 요청하지 않는 한 이 절차를 생략하지 않는다. 문서와 요청만으로 Label을 판단할 수 없으면 임의로 선택하지 말고 Issue 생성 전에 필요한 내용을 확인한다.

## 작업 순서

1. 새 기능이면 관련 PLAN 문서를 확인한다.
2. PLAN 또는 Issue에서 담당 영역, 대상 앱, 작업 범위, 개발 우선순위를 확인한다.
3. 예상 변경 파일과 작업 목적을 확인한다.
4. 영역·유형·우선순위 Label을 각각 선택한다.
5. Issue 생성 후 선택한 Label이 실제로 붙었는지 확인한다.

`type:feature`인데 기획서가 없으면 먼저 담당 영역의 `docs/templates/FEATURE_PLAN_TEMPLATE.md`를 사용해 기획서를 작성한다. 버그·문서·설정·리팩터링 작업에는 기획서를 강제하지 않는다.
...
- 영역 Label은 PLAN 또는 Issue의 대상 앱과 예상 변경 파일을 기준으로 선택한다. Frontend 담당 범위가 `apps/`여도 실제 대상에 따라 `area:desktop`, `area:extension`을 사용한다.
