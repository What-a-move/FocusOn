...
- 작업명: Issue #2 FastAPI 기반 AI 분석 서버 기본 구조 구현 전 계약 확인
- 담당 영역: AI
- 우선순위: 높음
- 상태: Issue 열림, 구현 전 검토 필요
...
5. Health Check, 공통 성공·실패 envelope, Schema 검증 테스트를 구현한다.
6. 로그에 원문·비밀 값이 남지 않는지 검사한다.
7. 기본 구조 완료 후 관련성 분석, Agent, 피드백, 노트, ColPali PoC를 별도 Issue로 진행한다.
