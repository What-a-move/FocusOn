# AI 작업 컨텍스트

> AI 담당자가 작업 시작 시 가장 먼저 읽는 문서다.
> 작업이 끝나면 현재 상태와 다음 담당자가 알아야 할 내용을 갱신한다.

## 담당 영역

- 학습 목표와 화면·페이지 정보의 관련성 판단
- OCR·페이지 텍스트 정제와 입력 품질 판단
- 페이지 이동·체류 시간을 이용한 목표 이탈 및 학습 흐름 분석
- MediaPipe가 클라이언트에서 계산한 상태값의 보조 분석
- 분석 결과의 상태값·점수·신뢰도 반환
- AI API 입력·출력 계약 관리
- 평가 데이터와 모델·프롬프트 품질 확인

## 현재 상태

### 완료된 작업

- AI 기본 디렉터리와 평가 디렉터리가 생성되어 있다.
- AI 전용 문서 구조와 기능별 템플릿을 구성했다.
- 공개 API 계약은 Notion `FocusOn API 명세서`, 로컬 요약은 루트 `docs/API_CONTRACT.md`에 정의되어 있다.
- AI 내부 호출 계약, 분석 기준, Prompt 가이드를 초안으로 작성했다.
- 관련성, 학습 흐름, MediaPipe 사용자 상태를 분리된 기능으로 개발하는 방향을 정리했다.

### 진행 중인 작업

- AI 실행 방식과 Python 의존성 확정
- Server·Desktop·Extension과 관련성 분석 요청·응답 계약 확인
- OCR 정제, 규칙, 임베딩, LLM Fallback의 세부 기준 확정
- 평가 데이터와 기준 결과 구성

### 아직 진행하지 않은 작업

- 실행 가능한 AI 서버 진입점 구현
- Pydantic 요청·응답 Schema 구현
- OCR·페이지 텍스트 정제와 품질 판단 구현
- 규칙 기반 관련성 판단 구현
- 임베딩 기반 유사도 판단 구현
- LLM Fallback 구현
- Server 연동
- 성능·정확도 평가 자동화
- 페이지 이동·체류 시간 기반 학습 흐름 분석
- MediaPipe 상태값 보조 분석
- YouTube 자막·PDF 추출 텍스트 연동

## 현재 기술

- 언어: Python
- API 서버: FastAPI 검토
- Schema 검증: Pydantic 검토
- 테스트: pytest 검토
- 모델: 임베딩 모델과 LLM 공급자 검토 필요
- 분석 대상: 화면·페이지에서 추출한 텍스트와 메타데이터
- OCR: 클라이언트가 로컬로 추출한 텍스트를 AI가 정제·평가
- 카메라 분석: MediaPipe 클라이언트 결과값만 후속 사용자 상태 분석에 사용
- 원본 저장: 원본 화면·카메라 영상은 기본 저장하지 않음

## 예상 데이터 흐름

```text
Desktop / Extension
  → 로컬 OCR 또는 페이지 텍스트 추출
  → Spring Server 검증·민감 정보 제거
  → AI 텍스트 정제
  → 규칙 기반 판단
  → 임베딩 판단
  → 애매한 경우 LLM 판단
  → Server 결과 저장
  → Desktop 상태·알림·리포트 표시
```

## 반드시 지켜야 하는 조건

- 입력 데이터에 비밀번호·결제 정보·개인 메시지가 포함되지 않도록 확인한다.
- 원본 화면·카메라 영상을 기본 저장하지 않는다.
- OCR 엔진 자체 개발과 원본 이미지 처리는 AI 담당 범위에 포함하지 않는다.
- 관련성 분석 API에는 체류 시간과 MediaPipe 상태값을 섞지 않는다.
- 학습 흐름과 사용자 상태는 별도 분석 기능으로 구현한다.
- 판단이 어려운 경우 `RELATED` 또는 `UNRELATED`로 단정하지 않고 `UNCERTAIN`을 반환한다.
- 규칙이나 임베딩으로 충분히 판단 가능한 요청에는 LLM을 호출하지 않는다.
- 모델·프롬프트·임계값 변경은 `DECISION_RECORD.md`에 기록한다.
- 분석 결과에는 최소한 상태값과 신뢰도 기준을 포함한다.
- 기능 추가 전 `features/` 안에 기획서를 먼저 작성한다.

## 작업 종료 시 갱신

- 완료·진행 중·미착수 작업
- 입력·출력 계약 변경
- 모델·프롬프트·임계값 결정
- 발생한 오류와 평가 결과
- 다음 작업과 선행 조건
